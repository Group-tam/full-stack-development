...
export default function AccountPage() {
	...
	const [avatarPreview, setAvatarPreview] = useState("/avatar-default.svg");
	...
	const handleAvatarUpload = (imageId: string) => {
		console.log(imageId)
		setAvatar(imageId);
		setAvatarPreview((imageId !== "000000000000000000000000")? `/user/image/${imageId}` : '/avatar-default.svg');
	};
	...
	useEffect(() => {
		if (user) {
			...
			// If user has a custom avatar, use the backend URL, otherwise use local default
			console.log(`vishy${user.avatar}`)
			setAvatarPreview((user.avatar !== "000000000000000000000000") ? `/user/image/${user.avatar}` : '/avatar-default.svg');
		}
	}, [user]);
	...
	const disabled=(
		Boolean(usernameError) || username === user?.username ||
		(currentPassword && (
			!canEditPassword ||
